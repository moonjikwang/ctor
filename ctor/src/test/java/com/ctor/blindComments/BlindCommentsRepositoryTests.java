...
public class BlindCommentsRepositoryTests {
	 ...
	 * blindc_no값을 블라인드 객체의 blindNo값으로 설정
	 */
	@Test
	public void addBComments() {
		IntStream.rangeClosed(1, 20).forEach(i -> {
			...
			Blind blind = Blind.builder().bno(blindCno).build();
			
			//blindCno값을 가진 블라인드객체를 블라인드코멘츠객체에 주입
			//공통키를 기준으로 블라인드에 존재하는 bno인 경우 데이터 Add
			BlindComments comments = BlindComments.builder()
					.blindCtext("익명댓글" + i)
					.blind(blind)
					.blindCreplyer("익명댓글작성자" + i)
					.build();
